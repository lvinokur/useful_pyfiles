...
lib.app.author = 'Lea Vinokur (lea.vinokur@florey.edu.au)'

lib.cmdlineParser.initialise('Calculate a fixel-template based on 100 HCP subjects. The analysis pipeline relies primarily on the MRtrix3 software package (www.mrtrix.org).')
...
#analysis_level_choices = ['subject_preproc', 'template_generate', ]
...
if lib.app.args.analysis_level == 'participant1':
  ...
  for subject_label in subjects_to_analyze:
    ...
    dwi_bias_file = s.path.join(subject_dir, 'dwi_preproc_bias.mif')
    lib.app.checkOutputFile(dwi_bias_file)
    ...
    # Compute brain mask
    runCommand('dwi2mask ' + dwi_preproc_file + ' ' + output_mask + lib.app.mrtrixForce)

    # Perform Bias Correction
    runCommand('dwibiascorrect' + lib.app.mrtrixNThreads + '-ants' + '-mask' + output_mask + ' ' + dwi_preproc_file + ' ' + dwi_bias_file)
...
# running group level 1 (perform intensity normalisation, calculate response functions, average response )
  ...
  printMessage('performing intensity normalisation')
  ...
  lib.app.checkOutputFile(fa_template)
  fa_wm_mask = os.path.join(template_dir, 'fa_wm_mask.mif')
  lib.app.checkOutputFile(fa_wm_mask)
  ...
  runCommand('dwiintensitynorm' + 'inorm_input' + 'mask_input' + intensitynorm_output + ' ' + fa_template + ' ' + fa_wm_mask )
